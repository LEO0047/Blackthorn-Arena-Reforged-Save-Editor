...
_CUSTOMTK_MODULE = "customtkinter"
_CUSTOMTK_SPEC = importlib.util.find_spec(_CUSTOMTK_MODULE)
if _CUSTOMTK_SPEC is None:
    _ERROR_MESSAGE = (
        "此版本介面需要安裝 CustomTkinter。\n\n"
    ...
    try:
        _root = tk.Tk()
        _root.withdraw()
        messagebox.showerror(APP_TITLE, _ERROR_MESSAGE)
        _root.destroy()
    except Exception:
        pass
    raise SystemExit(1)

ctk = importlib.import_module(_CUSTOMTK_MODULE)
